...
param (
    ...
    [switch]$DisableSettingsSync,
    [switch]$DisableOfficeTelemetry,
    [switch]$DisableErrorReporting,
    [switch]$DisableCEIP,
    [switch]$DisableCloudClipboard,
    [switch]$DisableMessageSync,
    [switch]$DisableLanguageListAccess,
    [switch]$DisableLockscreenCamera,
    [switch]$DisableSpeechModelUpdates,
    [switch]$DisableExperimentation,
    [switch]$DisableLLMNR,
    [switch]$DisableRemoteAssistance,
    [switch]$RestrictAnonymousAccess,
    [switch]$DisableWifiSense,
    [switch]$DisableWindowsConnectNow,
    ...
    [switch]$DisableNvidiaTelemetry,
    [switch]$DenyAppPermissions,
    [switch]$DisableDeviceMonitoring,
    [switch]$DisableProgramCompatibilityAssistant,
    [switch]$DisableCustomerExperienceImprovement,
    [switch]$DisablePrivacyExperience,
    [switch]$DisableRsopLogging,
    [switch]$DisableUserTracking,
    [switch]$DisallowMicrosoftAccounts,
    [switch]$DisableActivationTelemetry,
    [switch]$DisableDiagnosticTracing,
    [switch]$EnableDnsOverHttps,
    [switch]$DisableSmbBandwidthThrottling,
    [switch]$HideRecentFiles,
    [switch]$SetThumbnailCacheSize,
    [switch]$DisableDynamicLighting,
    [switch]$DisableVisualEffects,
    [switch]$DisableAeroShake,
    [switch]$DisableDesktopPeek,
    [switch]$UseCompactModeExplorer,
    [switch]$DisableCrossDeviceResume,
    [switch]$DisableSettingsTips,
    [switch]$EnableLongPaths,
    [switch]$UseClassicSearch,
    [switch]$DisableNearbySharing,
    [switch]$DisableNewsAndInterests,
    [switch]$DisableTabletMode,
    [switch]$DisableLowDiskWarning,
    [switch]$DisableStartupDelay,
    [switch]$DisableWpbt,
    [switch]$ConfigureQosReservation,
    [switch]$DisableNetworkThrottling,
    [switch]$DisableMsrtTelemetry,
    [switch]$SetBestWallpaperQuality,
    [switch]$DisableWarningSounds,
    [switch]$DisableItemCheckboxes,
    [switch]$DisableMenuDelay,
    [switch]$HideDisconnectedSounds,
    [switch]$RemoveShortcutSuffix,
    [switch]$EnableBlueTooltips,
    [switch]$DisableOverlayScrollbars,
    [switch]$DisableLoginBlur,
    [switch]$ShowBatteryPercentage,
    [switch]$DisableStoreAutoUpdates,
    [switch]$DisableUsbNotifications,
    [switch]$DisableSettingsBanner,
    [switch]$ShowMoreDetailsOnTransfer,
    [switch]$DisableInvalidShortcutSearch,
    [switch]$HideOfficeFilesInQuickAccess,
    [switch]$AlwaysFullContextMenu,
    [switch]$HideFrequentItems,
    [switch]$MinimizeMouseHoverTime,
    [switch]$DisableInternetOpenWith,
    [switch]$RemoveCastToDevice,
    [switch]$DisableTouchVisualFeedback,
    [switch]$ShowMorePins,
    [switch]$ShowAllControlPanelTasks,
    [switch]$DecreaseShutdownTime,
    [switch]$EnableVerboseMessages,
    [switch]$ForceEndShutdownApps,
    [switch]$ConfigureCrashControl,
    [switch]$WinXOpensCommandPrompt,
    [switch]$HideMeetNow,
    [switch]$DisableSoundReduction,
    [switch]$EnableDetailedBsod,
    [switch]$EnableGameMode,
    [switch]$DisableNewOutlook,
    [switch]$EnableNumLockOnBoot,
    [switch]$EnableS3Sleep
)
